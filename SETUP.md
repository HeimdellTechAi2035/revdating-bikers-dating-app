...
There are 25 migration files numbered 001–025.
...
- The default example model is `gpt-5.4-nano` because profile copywriting is a
  ...
  redeploy.
- The AI helpers keep the existing in-memory app rate limiter for burst
  protection and also write metadata-only usage events/counters when migration
  `025_ai_usage_tracking.sql` has been applied.
- AI safety v1 does not auto-ban users, auto-report users, create admin
  actions, or send messages on the user's behalf.
...
### 7a. Supabase AI usage tracking

AI usage tracking is backed by migration `025_ai_usage_tracking.sql`, which
creates three additive-only tables:

- `ai_usage_events` — append-only metadata events for AI usage, errors, plan,
  feature, and coarse risk classifications.
- `ai_usage_counters` — weekly/monthly per-user counters used for quota and
  abuse-prevention enforcement.
- `ai_feature_limits` — seeded free, Rider Plus, Rider Premium, and admin
  allowances for each AI feature.

The tracking tables are metadata-only. They must **never** store raw prompts,
private messages, generated AI outputs, full report descriptions, OpenAI
request/response payloads, private contact details, exact GPS coordinates, or
exact addresses. Store only safe metadata such as feature name, outcome, model,
character counts, output item counts, risk level/categories, and coarse related
entity type.

Migration `025_ai_usage_tracking.sql` is additive-only: it creates the AI
tracking tables, indexes, triggers, seed limit rows, and RLS policies without
altering existing REVdating tables. Production must **never** use destructive
reset commands such as `supabase db reset` or `npm run supabase:reset`.

Before production launch, verify:

- `supabase/migrations/025_ai_usage_tracking.sql` has been applied to the target
  Supabase project.
- `public.ai_usage_events`, `public.ai_usage_counters`, and
  `public.ai_feature_limits` exist.
- RLS is enabled on all three AI tracking tables.
- `ai_feature_limits` contains the seeded free, Rider Plus, Rider Premium, and
  admin allowance rows.
- Netlify has `OPENAI_API_KEY` and `SUPABASE_SERVICE_ROLE_KEY` set as
  server-side environment variables.
- `NEXT_PUBLIC_OPENAI_API_KEY` does not exist in Netlify or any committed env
  file.
